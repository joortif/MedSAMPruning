...
def prune_dataset(
    ...
    default_logits_path = (
        Path(output_path)
        / "logits"
        / f"logits_{pruning_model.lower()}.h5"
    )

    effective_logits_path = (
        Path(logits_path)
        if logits_path is not None
        else default_logits_path
    )
    ...
    if strategy not in ["forgetting", "cb-scs", "fusion"]:
        if effective_logits_path.exists():
            print(f"Using existing logits from: {effective_logits_path}")
        else:
        ...
        csv_cb_scs_path = Path(output_path) / "ranking" / "cb_scs.csv"

        csv_el2n_path = Path(output_path) / "ranking" / "el2n.csv"

        print(f"Calculating cb-scs and el2n scores for fusion strategy...")
        samples_ranking_cb_scs = cb_scs(mask_path=labels_dir, out_csv_path=csv_cb_scs_path)
        ...
        samples_ranking_el2n = compute_ranking_scores(image_path = images_dir, mask_path=labels_dir, logits_path=effective_logits_path,
                                                     metric_fn=METRICS["el2n"], metric_name="el2n", csv_path=csv_el2n_path)
        
        samples_ranking = fuse_cb_el2n(samples_ranking_cb_scs, samples_ranking_el2n, alpha=alpha, csv_path=csv_strategy_path)
    ...
    if samples_ranking is None:
        if strategy in ["el2n", "entropy"]:
            ...
            samples_ranking = compute_ranking_scores(image_path = images_dir, mask_path=labels_dir, logits_path=effective_logits_path,
        ...
        if strategy == "iou":
            ...
            samples_ranking = compute_iou_ranking(image_path=images_dir, mask_path=labels_dir, logits_path=effective_logits_path, 
